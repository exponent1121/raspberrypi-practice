...
## 코드 핵심 내용 (Code)
**1. 이벤트 기반 제어:** button.wait_for_press() 사용. 무한 루프 안에서 버튼 상태를 계속 묻는 비효율적인 방식이 아니라, 물리적인 이벤트가 발생할 때까지 대기하는 시스템 친화적인 코드 적용.

**2. 입출력 통합:** 입력(버튼)과 두 가지 다른 형태의 출력(상태 표시용 LED, 정보 전달용 7-Segment)을 하나의 사이클(run_timer) 안에서  통제.

**3. 예외 처리:** try...except...finally 구문을 사용해 사용자가 강제 종료 하더라도 하드웨어(LED) 상태를 안전하게 초기화(led.off())하고 끝내도록 설계.
